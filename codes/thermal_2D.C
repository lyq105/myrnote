...
void trianglelate()
{
 ...
 TriangleInterface t(mesh);
  ...
  t.smooth_after_generating() = true;
	...
	t.triangulate();
	TecplotIO(mesh).write("squre_hole.plt");
}

int triangle_circle (int argc, char** argv)

{
  ...
  libmesh_example_assert(2 <= LIBMESH_DIM, "2D support");
	...
	Mesh mesh(2);
		int n_outer_circle_points = 100;
	double outer_circle_ridus = 2;
	
	for(int i = 0; i< n_outer_circle_points; ++i)
	{
		double x = outer_circle_ridus * cos( i* 2 * libMesh::pi / n_outer_circle_points );
		double y = outer_circle_ridus * sin( i* 2 * libMesh::pi / n_outer_circle_points );
		mesh.add_point(Point (x , y));
	}
 ...
 TriangleInterface t(mesh);
  ...
  t.smooth_after_generating() = true;
	...
	t.triangulate();
	TecplotIO(mesh).write("anulus.plt");
	trianglelate(mesh2d);
	return 0;
}
*/
...
int main (int argc, char** argv)
{
 ...
 TriangleInterface t(mesh);
  ...
  mesh.find_neighbors();
	...
	std::cout << "write mesh file done!!\n" << std::endl;
	/*mesh.clear();*/
	TecplotIO(mesh).write("squre_tri.plt");
  ...
  // allow us to use higher-order approximation.
	...
	/*TecplotIO(mesh).write("squre_tri_2.plt");*/
  ...
  equation_systems.get_system("Poisson").add_variable("u", fe_type);
